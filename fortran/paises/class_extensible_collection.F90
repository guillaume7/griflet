...
!  !--------------end type-bound procedures type C_Colecao_Objecto------!
  ...
  function paraCadaNodo(self, node) result(keepup)
  ...
  function procuraId(self, id, nodo) result(found)
    ...
    if ( id .le. self%tamanho() ) then
      do while ( self%paraCadaNodo(item) )
  ...
  function procuraNodo(self, resultado, id, chave) result(found)
    class(C_Colecao)                    :: self
...
program unitTests_lista_colecao
  ...
  !e mostra elemento
  if ( caixa%procuraNodo( nodo, chave = str('Maçã especial') ) ) then
    call nodo%mostrarNodo()
  end if

  !Procura o elemento número 2 da lista,
  !extrai o valor e mostra
  if ( caixa%procuraNodo( nodo, id = 2 ) ) then
    call obterMaca( nodo, maca )
 ...
 ! end if
  ...
  nullify( nodo )
  do while( caixa%paraCadaNodo( nodo ) )
    call nodo%mostrarNodo( )
  enddo
